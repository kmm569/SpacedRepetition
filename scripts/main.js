...
document.body.addEventListener('change', event => {
    if (event.target !== testDate && event.target !== yearButton && event.target !== subjects && event.target.type !== 'number' && event.target.type !== 'date' && event.target.type !== 'text' && event.target.type !== 'range') {
        return;
    }
    sessionSave();
})
...
function saveConfig(saveName) {
  ...
  } else if (localStorage.getItem('saveList').split(',').indexOf(saveName) !== -1) {
  ...
  for (var k = 0; k < topicMonths.length; k++) {
      ...
      savedTopics.push(`${topicNames[k].value}((${topicYears[k].value}-${topicMonths[k].value}-${topicDays[k].value}((${topicDiff[k].value}`);
    ...
    localStorage.setItem('saveList', [localStorage.getItem('saveList'), saveName].join(','));
...
function getSaves() {
  ...
  saveInput.innerHTML = "";
    ...
    if (saves == null || saves.length == 0 || (saves.length == 1 && saves[0] == '')) {
...
function deleteSave(saveName) {
  ...
  localStorage.setItem('saveList', localStorage.getItem('saveList').split(',').splice(localStorage.getItem('saveList').split(',').indexOf(saveName), -1/* (localStorage.getItem('saveList').split(',').length == 1 ? -1 : 1) */).join(','));
}
